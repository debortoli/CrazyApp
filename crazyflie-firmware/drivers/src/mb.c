...
static float mb_distance = 0;

LOG_GROUP_START(mb)
LOG_ADD(LOG_FLOAT, distance, &mb_distance)
LOG_GROUP_STOP(mb)

float mb_read_distance(uint8_t pin)
{
  /*
  ...
  mb_distance = (float)(25.0 * analogRead(pin) / 5.12 /10.0);
  //mb_distance = (int)(mb_distance * 1000) / 1000;
  return mb_distance;
}
